...
graph_matrix = sp.sparse.csc_matrix(word_word_matrix)

combined_matrix = sp.sparse.hstack((pmi_matrix, graph_matrix))
#combined_matrix = graph_matrix
embeddingst, s, vt = sparsesvd(combined_matrix,k=100) 
...
embeddings_mod = np.absolute(embeddings)
print("max",np.max(embeddings_mod),"min",np.min(embeddings_mod))
