...
def worsim_show(embed, wordsim_path='data/wordsim353/combined.csv', ignoe_z=True):
    ws_data = get_data(wordsim_path)
    simscores = []
    for i in range(ws_data.shape[0]):
        try:
            simscores.append([ws_data[i, 0], ws_data[i, 1], embed.get_sim(ws_data[i, 0], ws_data[i, 1])])
        except KeyError:
            simscores.append([float(ws_data[i, 2]), 0])
